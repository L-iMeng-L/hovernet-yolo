...
def main():
    ...
    p.add_argument('--n_calls', type=int, default=30, help='Bayesian iterations')
    p.add_argument('--sample_size', type=int, default=100, help='Samples for search (0=all)')
    ...
    val_loader = get_dataloader(val_root, batch_size=args.batch_size, shuffle=False,
    ...
    # 搜索空间
    space = [
        Real(0.25, 0.45, name='np_thresh'),
        Real(0.30, 0.50, name='overall_thresh'),
        Integer(3, 7, name='marker_ksize'),
        Integer(2, 8, name='min_area')
    ]
    
    @use_named_args(space)
    def objective(**params):
        params['ksize'] = 21
        metrics = evaluate_params(model, val_loader, device, params, num_workers=8)
        return -metrics['PQ']
    ...
    best_params = {
        ...
        'ksize': 21,
        'marker_ksize': int(result.x[2]),
        'min_area': int(result.x[3])
    }
    best_pq = -result.fun
