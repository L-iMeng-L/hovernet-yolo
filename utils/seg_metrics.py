...
def get_fast_pq(true_inst, pred_inst, true_cls=None, pred_cls=None, match_iou=0.5):
    ...
    true_cls, pred_cls   : dict {inst_id: class_id}，可为None（只算检测指标）

    返回: dict
      检测指标: PQ, DQ, SQ, F1, Precision, Recall分类指标（需提供cls）: cls_acc（匹配对中分类正确率）
    """
    true_inst = _remap_label(true_inst)
    pred_inst = _remap_label(pred_inst)
    ...
    if n_true == 0 and n_pred == 0:
        return dict(PQ=1., DQ=1., SQ=1., F1=1., Precision=1., Recall=1., cls_acc=1.)
    ...
    # 分类准确率（匹配对中预测类别与GT类别一致的比例）
    cls_acc = 0.0
    if true_cls is not None and pred_cls is not None and tp > 0:
        correct = 0
        for ti, pi in zip(ri, ci):
            t_id = true_ids[ti]
            p_id = pred_ids[pi]
            if true_cls.get(t_id, -1) == pred_cls.get(p_id, -2):
                correct += 1
        cls_acc = correct / tp
...
def batch_seg_metrics(pred_inst_list, true_inst_list,
                      ...
                      match_iou=0.5):
    """
    pred_cls_list: list of dict {inst_id: class_id}，可为None
    true_cls_list: list of dict {inst_id: class_id}，可为None
    """
